...
function Editor({ note, noteID, content }) {
	...
	function onTagStringBlur(e) {
		...
		if (t.length <= 256 && t !== note.tags.join(" ")) {
			setTagString(t);
			setTags(tags);
	...
	if (!editAccess && !note.publicAccess) {
		...
		return (
			<div className="note-view">
				...
				<div
					className={`note-panel ${previewVisible ? "" : "--preview-hidden"
						}`}
				>
...
function ContentLoader({ note, noteID }) {
	...
	const { data: url } = useStorageDownloadURL(
		ref(storage, `${note.author}/${noteID}.md`)
	);
...
export default function EditorLoader({ noteID }) {
	...
	const {
		...
		data: note,
		error,
	} = useFirestoreDocData(doc(db, "notes", noteID));

	switch (status) {
		case "success": {
			if (
				note &&
				(note.publicAccess || auth.currentUser?.uid === note.author)
			) {
